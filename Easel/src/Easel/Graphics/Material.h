...
namespace Easel {
	...
	namespace Graphics {
		...
		struct MaterialProperties {
			...
			glm::vec4 metallicColour = glm::vec4(0.0f, 1.0f, 0.0f, 1.0f);
		...
		public:
			Material(Ref<Shader>& shader, const MaterialProperties& properties = MaterialProperties(), const PBRMaterialTextures& textures = PBRMaterialTextures());
			Material();
			...
			void load(Archive& archive)
			{
				...
				if (!albedoFilePath.empty())
					m_PBRMaterialTextures.albedo = Ref<Texture2D>(Texture2D::CreateFromFile("albedo", albedoFilePath));
				if (!normalFilePath.empty())
					m_PBRMaterialTextures.normal = Ref<Texture2D>(Texture2D::CreateFromFile("roughness", normalFilePath));
				if (!metallicFilePath.empty())
					m_PBRMaterialTextures.metallic = Ref<Texture2D>(Texture2D::CreateFromFile("metallic", metallicFilePath));
				if (!roughnessFilePath.empty())
					m_PBRMaterialTextures.roughness = Ref<Texture2D>(Texture2D::CreateFromFile("roughness", roughnessFilePath));
				if (!emissiveFilePath.empty())
					m_PBRMaterialTextures.emissive = Ref<Texture2D>(Texture2D::CreateFromFile("emissive", emissiveFilePath));
				if (!aoFilePath.empty())
					m_PBRMaterialTextures.ao = Ref<Texture2D>(Texture2D::CreateFromFile("ao", aoFilePath));
			}

		private:
			PBRMaterialTextures m_PBRMaterialTextures;
			Ref<Shader> m_Shader;
			Pipeline* m_Pipeline;
