...
namespace Easel {
	...
	bool VFS::ResolvePhysicalPath(const std::string& path, std::string& outPhysicalPath, bool folder) {
		...
		std::vector<std::string> dirs = StringUtils::SplitString(path, delimiter);
	...
	bool VFS::AbsolutePathToVFS(const std::string& path, std::string& outVFSPath, bool folder) {
