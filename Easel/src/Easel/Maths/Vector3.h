#pragma once
#include "Easel/Maths/Vector2.h"
#include "Easel/Maths/MathDefs.h"
#include <iostream>

namespace Easel::Maths {
	...
	public:
		...
		/// Construct from two-dimensional vector and the Z coordinate.
		...
		float AbsDotProduct(const Vector3& rhs) const {
			return Maths::Abs(x * rhs.x) + Maths::Abs(y * rhs.y) + Maths::Abs(z * rhs.z);
		}
		...
		Vector3 Abs() const { return Vector3(Maths::Abs(x), Maths::Abs(y), Maths::Abs(z)); }
		...
		bool Equals(const Vector3& rhs, float eps = M_EPSILON) const {
			return Maths::Equals(x, rhs.x, eps) && Maths::Equals(y, rhs.y, eps) && Maths::Equals(z, rhs.z, eps);
		}

		/// Returns the angle between this vector and another vector in degrees.
		float Angle(const Vector3& rhs)  const { return Maths::Acos(DotProduct(rhs) / (Length() * rhs.Length())); }

		/// Return whether is NaN.
		bool IsNaN() const { return Maths::IsNaN(x) || Maths::IsNaN(y) || Maths::IsNaN(z); }

		/// Return whether is Inf.
		bool IsInf() const { return Maths::IsInf(x) || Maths::IsInf(y) || Maths::IsInf(z); }
...
namespace std {
	...
	template<>
	struct hash<Easel::Maths::Vector3> {
		size_t operator()(const Easel::Maths::Vector3& x) const {
			return std::hash<float>()(x.x) ^ (hash<float>()(x.y) * 997u) ^ (hash<float>()(x.z) * 999983u);
		}
	};
}
