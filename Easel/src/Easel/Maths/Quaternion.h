...
namespace Easel::Maths {
	...
	public:
...
#if defined(EASEL_SSE) && (!defined(_MSC_VER) || _MSC_VER >= 1700) /* Visual Studio 2012 and newer. VS2010 bug */
		{
...
#if defined(EASEL_SSE) && (!defined(_MSC_VER) || _MSC_VER >= 1700) /* Visual Studio 2012 and newer. VS2010 has a bug with these, see https://github.com/Lumos/Lumos/issues/1044 */
		...
		bool Equals(const Quaternion& rhs, float eps = M_EPSILON) const {
			return Maths::Equals(w, rhs.w, eps) && Maths::Equals(x, rhs.x, eps) && Maths::Equals(y, rhs.y, eps) && Maths::Equals(z, rhs.z, eps);
		}

		/// Return whether any element is NaN.
		bool IsNaN() const { return Maths::IsNaN(w) || Maths::IsNaN(x) || Maths::IsNaN(y) || Maths::IsNaN(z); }

		/// Return whether any element is Inf.
		bool IsInf() const { return Maths::IsInf(w) || Maths::IsInf(x) || Maths::IsInf(y) || Maths::IsInf(z); }
