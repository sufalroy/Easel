...
namespace Easel::Maths {
	...
	public:
		...
		/// Copy-construct form another vector.
	...
	public:
		...
		float AbsDotProduct(const Vector2& rhs) const { return Maths::Abs(x * rhs.x) + Maths::Abs(y * rhs.y); }
		...
		Vector2 Abs() const { return Vector2(Maths::Abs(x), Maths::Abs(y)); }

		/// Linear interpolation with another vector.
		Vector2 Lerp(const Vector2& rhs, float t) const { return *this * (1.0f - t) + rhs * t; }

		/// Test for equality with another vector with epsilon.
		bool Equals(const Vector2& rhs, float eps = M_EPSILON) const { return Maths::Equals(x, rhs.x, eps) && Maths::Equals(y, rhs.y, eps); }

		/// Return whether is NaN.
		bool IsNaN() const { return Maths::IsNaN(x) || Maths::IsNaN(y); }
	...
	template<typename Archive> void serialize(Archive& archive, Vector2& v2) {
		archive(v2.x, v2.y);
	}
}
...
namespace std {
	...
	template<>
	struct hash<Easel::Maths::Vector2> {
		size_t operator()(const Easel::Maths::Vector2& x) const {
