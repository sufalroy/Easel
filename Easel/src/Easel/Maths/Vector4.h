...
namespace Easel::Maths {
	...
	public:
		...
		float AbsDotProduct(const Vector4& rhs) const {
			return Maths::Abs(x * rhs.x) + Maths::Abs(y * rhs.y) + Maths::Abs(z * rhs.z) + Maths::Abs(w * rhs.w);
		}
		...
		Vector4 Abs() const { return Vector4(Maths::Abs(x), Maths::Abs(y), Maths::Abs(z), Maths::Abs(w)); }
		...
		bool Equals(const Vector4& rhs, float eps = M_EPSILON) const {
			return Maths::Equals(x, rhs.x, eps) && Maths::Equals(y, rhs.y, eps) && Maths::Equals(z, rhs.z, eps) && Maths::Equals(w, rhs.w, eps);
		}

		/// Return whether is NaN.
		bool IsNaN() const { return Maths::IsNaN(x) || Maths::IsNaN(y) || Maths::IsNaN(z) || Maths::IsNaN(w); }
...
namespace std {
	template<>
	struct hash<Easel::Maths::Vector4> {
		size_t operator()(const Easel::Maths::Vector4& x) const noexcept {
