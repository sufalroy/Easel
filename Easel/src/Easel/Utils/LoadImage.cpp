...
namespace Easel {
	...
	uint8_t* LoadImageFromFile(const char* filename, uint32_t* width, uint32_t* height, uint32_t* bits, bool* isHDR, bool flipY, bool srgb) {
		...
		if (stbi_is_hdr(filename)) {
			sizeOfChannel = 16;
			pixels = (uint8_t*)stbi_load(filename, &texWidth, &texHeight, &texChannels, 0);
		...
		EASEL_ASSERT(pixels, "Could not load image '{0}'!", filename);

		//TODO: support different texture channels
		if (texChannels != 4)
			texChannels = 4;
