...
namespace Easel {
	...
	namespace Graphics {
		...
		GLenum GLFramebuffer::GetAttachmentPoint(TextureFormat format) {
			EASEL_PROFILE_FUNCTION();
		...
		void GLFramebuffer::AddTextureAttachment(const TextureFormat format, Texture* texture) {
			EASEL_PROFILE_FUNCTION();
		...
		void GLFramebuffer::AddCubeTextureAttachment(TextureFormat format, CubeFace face, TextureCube* texture) {
			EASEL_PROFILE_FUNCTION();
			uint32_t faceID = 0;
		...
		void GLFramebuffer::Validate() {
			...
			if (status != GL_FRAMEBUFFER_COMPLETE) {
				EASEL_CORE_CRITICAL("Unable to create Framebuffer! StatusCode: {0}", status);
			}
		}
