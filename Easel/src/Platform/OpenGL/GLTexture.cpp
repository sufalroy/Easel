...
namespace Easel {
	...
	namespace Graphics {
		...
		void GLTexture2D::BuildTexture(TextureFormat internalformat, uint32_t width, uint32_t height, bool srgb, bool depth, bool samplerShadow) {
			m_Width = width;
			m_Height = height;
			...
			if (samplerShadow) {
				GLCall(glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_COMPARE_FUNC, GL_EQUAL));
			...
			GLCall(glTexImage2D(GL_TEXTURE_2D, 0, Format, width, height, 0, Format2, depth ? GL_UNSIGNED_BYTE : GL_FLOAT, nullptr));
		}
		...
		uint8_t* GLTexture2D::LoadTextureData() {
			...
			if (m_FileName != "NULL") {
				uint32_t bits;
				pixels = Easel::LoadImageFromFile(m_FileName.c_str(), &m_Width, &m_Height, &bits, &isHDR, !m_LoadOptions.flipY);
			...
			, m_Format()
		{
			...
			//GLCall(glTexStorage2D(m_Handle, m_NumMips, GL_RGBA16F, m_Size, m_Size));

#ifndef EASEL_PLATFORM_MOBILE
			GLCall(glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP));
			GLCall(glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP));
			GLCall(glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_R, GL_CLAMP));
#endif // !EASEL_PLATFORM_MOBILE
			...
			//Set textures
			for (int i = 0; i < 6; ++i)
				GLCall(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_RGBA16F, size, size, 0, GL_RGBA, GL_FLOAT, nullptr));
		...
		GLTextureCube::GLTextureCube(const std::string* files, uint32_t mips, TextureParameters params, TextureLoadOptions loadOptions, InputFormat format) {
		...
		uint32_t GLTextureCube::LoadFromMultipleFiles() {
			...
			bool isHDR = false;
			uint8_t* xp = Easel::LoadImageFromFile(xpos, &width, &height, &bits, &isHDR, true);
			uint8_t* xn = Easel::LoadImageFromFile(xneg, &width, &height, &bits, &isHDR, true);
			uint8_t* yp = Easel::LoadImageFromFile(ypos, &width, &height, &bits, &isHDR, true);
			uint8_t* yn = Easel::LoadImageFromFile(yneg, &width, &height, &bits, &isHDR, true);
			uint8_t* zp = Easel::LoadImageFromFile(zpos, &width, &height, &bits, &isHDR, true);
			uint8_t* zn = Easel::LoadImageFromFile(zneg, &width, &height, &bits, &isHDR, true);
		...
		uint32_t GLTextureCube::LoadFromVCross(uint32_t mips) {
			...
			for (uint32_t m = 0; m < mips; m++) {
				bool isHDR = false;
				uint8_t* data = Easel::LoadImageFromFile(m_Files[m], &srcWidth, &srcHeight, &bits, &isHDR, !m_LoadOptions.flipY);
				m_Parameters.format = BitsToTextureFormat(bits);
